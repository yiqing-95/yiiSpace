...
class Net_Gearman_Client
{
    ...
    protected function submitTask(Net_Gearman_Task $task)
    {
        ...
        $socket_key = $this->_getSocketAddr($s);

        if (!isset(Net_Gearman_Connection::$waiting[$socket_key]) || !is_array(Net_Gearman_Connection::$waiting[$socket_key])) {
            Net_Gearman_Connection::$waiting[$socket_key] = array();
        }

        array_push(Net_Gearman_Connection::$waiting[$socket_key], $task);
    }
    ...
    protected function handleResponse($resp, $s, Net_Gearman_Set $tasks)
    {
        ...
        case 'job_created':
            $socket_key = $this->_getSocketAddr($s);
            $task         = array_shift(Net_Gearman_Connection::$waiting[$socket_key]);

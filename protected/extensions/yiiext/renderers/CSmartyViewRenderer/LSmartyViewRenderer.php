...
class CSmartyViewRenderer extends CApplicationComponent implements IViewRenderer  {
    ...
    function init(){
        ...
        $this->_smarty->plugins_dir[] = Yii::getPathOfAlias('application.extensions.Smarty.plugins');
        if(!empty($this->pluginsDir)){
            $this->_smarty->plugins_dir[] = Yii::getPathOfAlias($this->pluginsDir);
        }

        if(!empty($this->configDir)){
            $this->_smarty->config_dir = Yii::getPathOfAlias($this->configDir);
        }

        $this->_smarty->assign("TIME", sprintf('%0.5f',Yii::getLogger()->getExecutionTime()));
        $this->_smarty->assign("MEMORY", round(Yii::getLogger()->getMemoryUsage()/(1024*1024),2)." MB");

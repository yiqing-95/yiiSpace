 ...
 *   临时修改   如果bug得到纠正  会用原始的呢
 * @version 1.5
 * @package yiiext.behaviors.model.taggable
 */
class ETaggableBehavior2 extends CActiveRecordBehavior {
	/**
	...
	public function getAllTags($criteria = null) {
        $extraCacheKey = empty($criteria) ? '' : crc32(serialize($criteria));
	...
	public function getAllTagsWithModelsCount($criteria = null) {
        $extraCacheKey = empty($criteria) ? '' : crc32(serialize($criteria));
		if(!($tags = $this->cache->get('Taggable'.$this->getOwner()->tableName().'AllWithCount'.$extraCacheKey))){
			// getting associated tags
			$builder = $this->getOwner()->getCommandBuilder();
			...
			$this->cache->set('Taggable'.$this->getOwner()->tableName().'AllWithCount'.$extraCacheKey, $tags);
		}

...
class <?php echo $this->controllerClass; ?> extends <?php echo $this->baseControllerClass."\n"; ?>
{
    ...
    */
     ...
     * @param CModel $newModel
     * @param CModel $oldModel
     * @param CWebUser $userCtx
     * @throws CHttpException
     * ------------------------
     * 只有model主人才可以更新 当然delete也可以
     * 如果主人验证的逻辑出现在model中 比如beforeSave beforeDelete 中
     * 可能需要判断应用程序的类型： 前端、后端 还是console
     */
    protected function validateModelUpdate(CModel $newModel,CModel $oldModel=null,CWebUser $userCtx=null){
        if (isset($newModel->uid)) {
            if($newModel->uid != $userCtx->id) {
               throw new CHttpException(403,"You may only update this model with user {$userCtx->name}");
            }
        }
    }

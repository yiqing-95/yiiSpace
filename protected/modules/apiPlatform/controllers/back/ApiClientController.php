...
class ApiClientController extends BackendController
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['ApiClient']))
		{
			$model->attributes=$_POST['ApiClient'];
			if($model->save())
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new ApiClient)
     */
			...
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    exit(CJSON::encode(array(
                            'status' => 'success',
                            'message' => "ApiClient successfully saved"
                        )
                    ));
	...
	{
		$dataProvider=new CActiveDataProvider('ApiClient');
	...
	{
		$model=new ApiClient('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ApiClient']))
			$model->attributes=$_GET['ApiClient'];
	...
	{
		$model=ApiClient::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='api-client-form')
		{
    ...
    public function actionBatchUpdateAjax()
	{
		...
		{
            $model->attributes=$_POST['ApiClient'];
			if($model->validate(array_keys($_POST['ApiClient']))){
                     $items=$this->getItemsToUpdate();
                    foreach($items as $i=>$item)
                    {
                        $item->attributes = $_POST['ApiClient'];
                        $item->save(false);// $item->save(); will run the validate function !
                    }
                    exit(CJSON::encode(array(
                            'status' => 'success',
                            'message' => "ApiClient successfully saved" // .print_r($_POST['Comment'],true),
                        )
                    ));
            }
		}
    ...
    */
    public  function getItemsToUpdate(){
        ...
        $items = ApiClient::model()->findAll($criteria);

...
(function($) {
	...
	$dialog.find('form').submit(function(){
        ...
        if($.fn.commentsList.settings[id]['dialogOptions']){
           dialogOptions = $.extend(defaultDialogOptions,$.fn.commentsList.settings[id]['dialogOptions'] || {});
        }else{
    ...
    $.fn.commentsList.postComment = function($dialog){
        ...
        function(data){
                ...
                {
                    // 这个方法在cmtList 视图定义
                    //refreshCmtListWithCurrentPage();
                    refreshCmtList();
                    $dialog.dialog("close");
                }
            },'json');
       /*
        * $.parsJSON will cause some problem！
        $.post(
            $form.attr("action"),
            $form.serialize()
            ).success(function(data){
